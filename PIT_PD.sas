...
/* Removing customers with less than 1 year data which can not form the observation window of 1 year as required by IFRS9 */
PROC SQL;
	create table mydata1 as select * from mydata where cust_id in
(select cust_id from 
(select cust_id, count(*) as count from mydata group by cust_id having 
		calculated count ge 4));
QUIT;
...
ods html file='/home/u49308301/PhD/validate.xlsx'; 
%let Noofrecs = 18248; 
%let Noofbins = 10; 
data pred_default; 
set train3; 
...
logit = 2.2741 +  -21.2035 * remaining_term_band_ +  36.5747 * loan_term_band_woe +  -60.7317 * ir_band_woe 
+  509.8 * hpi_lag_band_woe +  -280.9 * gdp_lag_band_woe +  -15.9987 * gdp_band_woe +  -4.1325 * bureau_band_woe ; 
